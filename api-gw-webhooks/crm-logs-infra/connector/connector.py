    ...
    try:
        ...
        entries = [
            {
                ...
                "PartitionKey": str(
                    record.get("id", hash(json.dumps(record)))
                ),  # Use id as partition key if available
            }
        ...
        if failed:
            ...
            failed_records = [
                (
                    i,
                    response["Records"][i].get("ErrorCode"),
                    response["Records"][i].get("ErrorMessage"),
                )
            ...
            logger.info(
                f"Successfully sent {len(records)} records to Kinesis Data Stream."
            )
...
def main():
    logger.info(
        f"Starting connector loop. Sending to Kinesis Data Stream: {STREAM_NAME}"
    )
    buffer = []
