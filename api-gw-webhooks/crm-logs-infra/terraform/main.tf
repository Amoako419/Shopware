...
resource "aws_iam_role_policy" "apigw_firehose_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = aws_kinesis_firehose_delivery_stream.crm_firehose_stream.arn
    }]
  })
}
...
resource "aws_lambda_function" "proxy" {
  ...
  environment {
    variables = {
      DELIVERY_STREAM = aws_kinesis_firehose_delivery_stream.crm_firehose_stream.name
    }
  }
}
...
output "delivery_stream" {
  value       = aws_kinesis_firehose_delivery_stream.crm_firehose_stream.name
  description = "Kinesis Firehose delivery stream name"
}
