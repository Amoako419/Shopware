...
def lambda_handler(event, context):
    try:
        ...
        if not crm_logs.empty and 'interaction_type' in crm_logs.columns and 'rating' in crm_logs.columns:
        ...
        if not web_logs.empty:
            ...
            session_group['session_duration'] = session_group['end_time'] - session_group['start_time']
            ...
            if not session_group.empty:
                ...
                if hasattr(avg_duration, 'total_seconds'):
                    ...
                    avg_duration_seconds = float(avg_duration) if pd.notnull(avg_duration) else 0
                    
                bounce_rate = (session_group['is_bounce'].sum() / len(session_group) * 100) if len(session_group) > 0 else 0
