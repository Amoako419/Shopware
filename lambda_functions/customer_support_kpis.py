    ...
    try:
        ...
        for i, record in enumerate(event["Records"]):
            try:
                ...
                logger.debug(
                    f"Record {i+1} sample: customer_id={payload.get('customer_id', 'N/A')}, "
        ...
        if not crm_logs.empty:
            ...
            logger.debug(
                f"Sample CRM log: {crm_logs.iloc[0].to_dict() if len(crm_logs) > 0 else 'N/A'}"
            )
        else:
        ...
        if not crm_logs.empty:
            ...
            logger.info(
                f"Dropped {original_count - len(crm_logs)} records missing customer_id or timestamp"
            )
            ...
            logger.debug(
                f"Converted timestamps to datetime; sample interaction_time: {crm_logs['interaction_time'].iloc[0] if len(crm_logs) > 0 else 'N/A'}"
            )
            ...
            if "rating" in crm_logs.columns:
                ...
                invalid_rating_count = len(
                    crm_logs[~valid_ratings & crm_logs["rating"].notna()]
                )
                crm_logs = crm_logs[valid_ratings | crm_logs["rating"].isna()]
                logger.info(
                    f"Filtered {invalid_rating_count} records with invalid ratings (not 1-5)"
                )
            logger.info(f"After cleaning, {len(crm_logs)} CRM log records remain")
            logger.info(
                f"Transformation completed in {(datetime.utcnow() - transform_start).total_seconds()} seconds"
            )
        else:
        ...
        if not crm_logs.empty and "interaction_type" in crm_logs.columns:
            resolution_interactions = crm_logs[
                crm_logs["interaction_type"] == "Resolution"
            ]
            ...
            if not resolution_interactions.empty:
                ...
                sorted_logs["prev_interaction_time"] = sorted_logs.groupby(
                    "customer_id"
                )["interaction_time"].shift(1)
                resolution_times = sorted_logs[
                    sorted_logs["interaction_type"] == "Resolution"
                ]
                resolution_times = resolution_times[
                    resolution_times["prev_interaction_time"].notna()
                ]
                logger.info(
                    f"Found {len(resolution_times)} valid Resolution interactions with prior interactions"
                )
                if not resolution_times.empty:
                    resolution_times["time_to_resolve"] = (
                        resolution_times["interaction_time"]
                        - resolution_times["prev_interaction_time"]
                    ).dt.total_seconds() / 60.0
                    avg_time_to_resolution = resolution_times["time_to_resolve"].mean()
                    logger.info(
                        f"Calculated Avg Time-to-Resolution: {avg_time_to_resolution:.2f} minutes"
                    )
                else:
                    logger.info(
                        "No valid Resolution interactions for Time-to-Resolution calculation"
                    )
            else:
                logger.info(
                    "No Resolution interactions for Time-to-Resolution calculation"
                )
        ...
        try:
            logger.info(
                f"Using Redshift Cluster ID: {REDSHIFT_CLUSTER_ID}, Database: {REDSHIFT_DB}, User: {REDSHIFT_USER}"
            )
            ...
            logger.debug(
                f"Redshift parameters: {[p['name'] + '=' + p['value'] for p in parameters_list]}"
            )
            ...
            logger.info(
                f"Redshift write completed in {(datetime.utcnow() - redshift_start).total_seconds()} seconds"
            )

            if len(crm_logs) > 0:
                logger.info(
                    f"Storing {len(crm_logs)} CRM logs in Redshift (placeholder)"
                )
                # Placeholder for raw data storage logic

            total_duration = (datetime.utcnow() - start_time).total_seconds()
            logger.info(
                f"Lambda execution completed successfully in {total_duration} seconds"
            )
