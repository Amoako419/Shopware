...
def process_records(records, required_fields):
    ...
    for record in records:
        ...
        try:
            ...
            if validate_record(json_data, required_fields):
                ...
                processed_records.append(
                    {
                        ...
                        "data": record[
                            "data"
                        ],  # Return original data for Firehose to process
                    }
                )
                valid_count += 1
            else:
                ...
                logger.warning(
                    f"Dropped record due to null required fields: {json_data}"
                )
                processed_records.append(
                    {
                        "recordId": record_id,
                        ...
                        "data": record[
                            "data"
                        ],  # Return original data but mark as Dropped
                    }
                )
                dropped_count += 1
    ...
    logger.info(
        f"Processed {len(records)} records: {valid_count} valid, {dropped_count} dropped"
    )
