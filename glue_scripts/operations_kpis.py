...
from pyspark.sql.functions import avg, col, count, from_unixtime, lit, sum, when
...
try:
    dyf_pos = glueContext.create_dynamic_frame.from_catalog(
        database=POS_DATABASE, table_name=POS_TABLE, additional_options={"version": "2"}
    )

    dyf_inv = glueContext.create_dynamic_frame.from_catalog(
        database=POS_DATABASE, table_name=INV_TABLE, additional_options={"version": "4"}
    )
    ...
    sales_per_product = df_pos.groupBy("product_id").agg(
        count("*").alias("transactions"), sum("quantity").alias("units_sold")
    )
    ...
    ).withColumn("inventory_turnover", col("units_sold") / col("avg_stock_level"))
    ...
    restock_events = (
        ...
        .groupBy("product_id")
        .agg(sum("restock_event").alias("restock_frequency"))
    )
    ...
    sold_out_products = (
        df_pos.select("product_id").distinct().join(stockouts, "product_id", "inner")
    )
    ...
    # Combine All Metrics
    final_metrics = (
        inventory_turnover.join(restock_events, on="product_id", how="left")
        .join(stockout_alerts, on="product_id", how="left")
        .na.fill(0)
    )
    ...
    final_metrics.write.format("delta").mode("overwrite").save(delta_partitioned_path)
