...
from pyspark.sql.functions import col, avg, count, max, min, from_unixtime
...
def compute_feedback_score(df):
    ...
    try:
        ...
        feedback_score = feedback_df.agg(avg("rating").alias("avg_feedback_score")).collect()[0]["avg_feedback_score"]
...
def compute_interaction_volume_by_type(df):
    ...
    try:
        ...
        volume_dict = {row["interaction_type"]: row["volume"] for row in volume_df.collect()}
...
def compute_time_to_resolution(df):
    ...
    try:
        ...
        complaint_df = complaint_df.withColumn("timestamp_dt", from_unixtime(col("timestamp")).cast(TimestampType()))
        time_diff = complaint_df.groupBy("customer_id").agg(
            (max("timestamp") - min("timestamp")).cast("long").alias("resolution_time")
        )
        avg_resolution = time_diff.agg(avg("resolution_time").alias("avg_resolution_time")).collect()[0]["avg_resolution_time"]
...
def write_kpis_to_s3(spark, kpis, output_path):
    ...
    try:
        ...
        kpi_df = spark.createDataFrame([kpis], schema=[
            "feedback_score",
            "interaction_volume",
            "time_to_resolution",
            "timestamp"
