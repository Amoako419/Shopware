...
def read_data(spark, pos_path, inventory_path, pos_schema, inventory_schema):
    ...
    try:
        pos_df = (
            spark.read.schema(pos_schema)
            .parquet(pos_path)
            .filter(to_date(from_unixtime(col("timestamp"))).isNotNull())
        )
        logger.info(
            f"Successfully read POS data from {pos_path} with {pos_df.count()} records."
        )

        inventory_df = (
            spark.read.schema(inventory_schema)
            .parquet(inventory_path)
            .filter(to_date(from_unixtime(col("last_updated"))).isNotNull())
        )
        logger.info(
            f"Successfully read Inventory data from {inventory_path} with {inventory_df.count()} records."
        )
...
def compute_total_sales(pos_df):
    ...
    try:
        total_sales_df = (
            pos_df.withColumn("date", to_date(from_unixtime(col("timestamp"))))
            .groupBy("date", col("store_id"), col("product_id"))
...
def compute_stock_availability(inventory_df):
    ...
    try:
        stock_availability_df = (
            inventory_df.withColumn("date", to_date(from_unixtime(col("last_updated"))))
            .groupBy("date", col("product_id"))
            .agg(
                avg(
                    when(
                        col("stock_level")
                        > coalesce(col("restock_threshold"), lit(0.0)),
                        1.0,
                    ).otherwise(0.0)
                    * 100
                ).alias("stock_availability")
            )
        )
...
def compute_product_turnover(pos_df, inventory_df):
    ...
    try:
        ...
        turnover_df = (
            units_sold_df.join(avg_stock_df, ["date", "product_id"], "inner")
            .withColumn(
                "turnover_rate", round((col("units_sold") / col("avg_stock_level")), 3)
            )
...
def save_to_redshift(spark, df, jdbc_url, redshift_user, redshift_password, table_name):
    ...
    try:
        ...
        df.write.format("jdbc").option("url", jdbc_url).option(
            "dbtable", table_name
        ).option("user", redshift_user).option("password", redshift_password).option(
...
def main():
    ...
    try:
        ...
        # Read data
        pos_df, inventory_df = read_data(
            spark, pos_path, inventory_path, pos_schema, inventory_schema
        )
        ...
        kpi_df = (
            total_sales_df.alias("sales")
            .join(
                stock_availability_df.alias("stock"),
                on=["date", "product_id"],
                how="outer",
            )
            .join(turnover_df.alias("turnover"), on=["date", "product_id"], how="outer")
            .select(
                col("date"),
